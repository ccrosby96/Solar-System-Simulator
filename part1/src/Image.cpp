...
void Image::loadJPG(bool flip){
    std::cout << "Loading jpeg file: "<< m_filepath.c_str() << std::endl;
    m_pixelData = stbi_load(m_filepath.c_str(), &m_width, &m_height, &nrChannels, 0);

    std::cout << "width from jpeg:" << m_width << std::endl;
    std::cout << "height from jpeg:" << m_height<< std::endl;
